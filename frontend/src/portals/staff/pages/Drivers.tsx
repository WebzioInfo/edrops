...
export interface StaffDriverDetailRecord extends StaffDriverRecord {
  orders?: {
    id: string;
    status: string;
    totalAmount: number;
...
export default function StaffDrivers() {
  const [drivers, setDrivers] = useState<StaffDriverRecord[]>([]);
  const [distributorsList, setDistributorsList] = useState<{ id: string; name: string }[]>([]);
  ...
  const [editingDriver, setEditingDriver] = useState<StaffDriverRecord | null>(null);
  ...
  const [driverDetail, setDriverDetail] = useState<StaffDriverDetailRecord | null>(null);
  const [isLoadingDetail, setIsLoadingDetail] = useState<boolean>(false);

  // Delete / Deactivate dialog state
  const [deleteTarget, setDeleteTarget] = useState<StaffDriverRecord | null>(null);
  const [isDeleting, setIsDeleting] = useState<boolean>(false);

  // Load distributors for filter & assignment dropdown
  const loadDistributorsList = async () => {
    try {
      const data = await fetchWithAuth('/staff/distributors');
      if (Array.isArray(data)) {
        const formatted = data.map((d: any) => {
          const agency = d.agencyName || d.distributor?.agencyName;
          const fullName = `${d.firstName || ''} ${d.lastName || ''}`.trim() || 'Distributor';
          const label = agency ? `${agency} (${fullName})` : fullName;
          return { id: d.id, name: label };
        });
        setDistributorsList(formatted);
      }
    } catch {
      // ignore
    }
  ...
  const loadDrivers = async (showLoadingSpinner = true) => {
    try {
      ...
      const data = await fetchWithAuth(`/staff/drivers${qs}`);
  ...
  }, [search, statusFilter, distributorFilter]);

  // Load single driver detail
  const handleOpenDetail = async (driverId: string) => {
    setViewingDriverId(driverId);
    setIsLoadingDetail(true);
    try {
      const data = await fetchWithAuth(`/staff/drivers/${driverId}`);
  ...
  const handleOpenCreateModal = () => {
    ...
    setDistributorId(distributorsList.length > 0 ? distributorsList[0].id : '');
    setName('');
    setPhone('');
    setAlternatePhone('');
    setRouteOrArea('');
  ...
  const handleOpenEditModal = (driver: StaffDriverRecord) => {
    setEditingDriver(driver);
    setDistributorId(driver.distributorId);
    setName(driver.name);
  ...
  const validateForm = () => {
    ...
    if (!editingDriver && !distributorId) {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingDriver) {
        await fetchWithAuth(`/staff/drivers/${editingDriver.id}`, {
          method: 'PUT',
          body: JSON.stringify(payload),
        });
        showToast.success('Driver updated successfully');
      } else {
        await fetchWithAuth('/staff/drivers', {
          method: 'POST',
          body: JSON.stringify(payload),
        });
  ...
  const handleToggleStatus = async (driver: StaffDriverRecord) => {
    try {
      const nextStatus = !driver.isActive;
      await fetchWithAuth(`/staff/drivers/${driver.id}/status`, {
        method: 'PATCH',
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      const res = await fetchWithAuth(`/staff/drivers/${deleteTarget.id}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5 bg-white p-3 rounded-xl border border-[#E2E8F0] shadow-xs">
        ...
        <div className="flex flex-wrap items-center gap-2 flex-1 min-w-0">
          ...
          <div className="relative min-w-[200px] flex-1">
            ...
            <input
              ...
              placeholder="Search driver, vehicle, phone, or distributor..."
          ...
          <div className="min-w-[170px]">
            <select
              ...
              className="w-full py-1.5 px-2.5 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg text-xs font-medium text-[#16324F] focus:outline-none focus:ring-1 focus:ring-[#1677C8]"
            >
          ...
          {/* Status Tabs */}
          <div className="flex items-center gap-1 p-1 bg-[#F1F5F9] rounded-lg shrink-0">
            {(
              [
        ...
        {/* Right Side: Refresh & Add Driver */}
        <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto">
          <button
            type="button"
            onClick={() => loadDrivers(false)}
            disabled={isRefreshing}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-700 bg-white border border-[#E2E8F0] hover:bg-slate-50 rounded-lg transition shadow-xs disabled:opacity-50 cursor-pointer"
            title="Refresh list"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white bg-[#1677C8] hover:bg-[#1263a8] rounded-lg transition shadow-xs cursor-pointer"
          >
      ...
      {isLoading ? (
        ...
        <div className="py-16 px-4 bg-white rounded-xl border border-[#E2E8F0] text-center space-y-3">
          ...
          <div>
            <p className="font-bold text-sm text-[#16324F]">No drivers found</p>
            <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
              {search || statusFilter !== 'ALL' || distributorFilter !== 'ALL'
                ? 'No drivers matched your active search or filters. Try adjusting filter options.'
                : 'No delivery drivers registered in the system yet.'}
            </p>
          </div>
          <button
            type="button"
            ...
            <table className="w-full text-left text-xs text-[#16324F]">
              <thead className="bg-[#F8FAFC] border-b border-[#E2E8F0] font-semibold text-slate-500 uppercase tracking-wider text-[11px]">
                <tr>
                  ...
                  <th className="py-3 px-4">Owning Distributor</th>
              ...
              <tbody className="divide-y divide-[#E2E8F0]">
                {drivers.map((driver) => {
                  ...
                  const distAgency = driver.distributor?.distributor?.agencyName;
                  const distName = driver.distributor
                    ? `${driver.distributor.firstName || ''} ${driver.distributor.lastName || ''}`.trim()
                    : 'Unknown Distributor';
                    ...
                    <tr key={driver.id} className="hover:bg-slate-50/70 transition-colors">
                          ...
                          <div className="min-w-0">
                            ...
                            {driver._count?.orders !== undefined && driver._count.orders > 0 && (
                              <span className="inline-flex items-center gap-1 text-[10px] text-slate-500">
                                ...
                                {driver._count.orders} {driver._count.orders === 1 ? 'order' : 'orders'}
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-1.5">
                          ...
                          <div className="min-w-0">
                            ...
                            {distAgency && distName && (
                              <p className="text-[10px] text-slate-400 truncate">{distName}</p>
                            )}
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-4">
                        <a
                          ...
                          className="inline-flex items-center gap-1 font-semibold text-slate-700 hover:text-[#1677C8]"
                        >
              ...
              return (
                <div key={driver.id} className="p-3.5 bg-white space-y-2.5">
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
                    ...
                    {driver.phone && (
                      <a href={`tel:${driver.phone}`} className="text-slate-600 hover:text-[#1677C8]">
                        📞 {driver.phone}
                      </a>
                    )}
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs animate-in fade-in duration-150">
          <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl border border-[#E2E8F0] overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm font-bold text-[#16324F]">
                    {editingDriver ? 'Edit Driver' : 'Add New Driver (Staff Action)'}
                  </h3>
                  <p className="text-[11px] text-slate-500">
                    {editingDriver
                      ? 'Update driver credentials and vehicle assignment'
                      : 'Create a delivery driver for a specific distributor'}
                  </p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleFormSubmit} className="flex-1 overflow-y-auto p-5 space-y-4">
              {/* Owning Distributor Selection (only for create) */}
              {!editingDriver && (
                <div>
                  <label className="text-xs font-semibold text-slate-700 block mb-1">
                    Owning Distributor <span className="text-rose-500">*</span>
                  </label>
                  <select
                    value={distributorId}
                  ...
                  {formErrors.distributorId && (
                    <p className="text-[11px] text-rose-500 mt-0.5">{formErrors.distributorId}</p>
                  )}
                </div>
              )}
              ...
              <div className="pt-2 border-t border-slate-100 space-y-3">
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="e.g. Full-time staff driver"
      ...
      {/* ─── DRIVER DETAIL DRAWER / MODAL (STAFF) ───────────────────── */}
      {viewingDriverId && (
          ...
          <div className="w-full max-w-2xl bg-white rounded-2xl shadow-xl border border-[#E2E8F0] overflow-hidden flex flex-col max-h-[92vh]">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#E2E8F0] bg-[#F8FAFC]">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-sm font-bold text-[#16324F]">Driver Details & Assignment Audit</h3>
                  <p className="text-[11px] text-slate-500">
                    Cross-distributor driver profile and delivery activities
                  </p>
                </div>
              </div>
              <button
                ...
                <>
                  ...
                  <div className="p-4 rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] space-y-3">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 pt-3 border-t border-[#E2E8F0] text-xs">
                      <div>
                        <span className="text-[11px] text-slate-400 block">Owning Distributor</span>
                        <span className="font-bold text-[#1677C8]">
                          {driverDetail.distributor?.distributor?.agencyName ||
                            `${driverDetail.distributor?.firstName || ''} ${driverDetail.distributor?.lastName || ''}`.trim() ||
                            'Distributor'}
                        </span>
                      </div>
                      <div>
                        ...
                        <span className="font-mono font-bold text-[#16324F]">
                          {driverDetail.vehicleType} • {driverDetail.vehicleNumber}
                        </span>
                      </div>
                      <div>
                  ...
                  <div className="space-y-2.5">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-slate-500 font-semibold">
                        {driverDetail.orders?.length || 0} assignments
                      </span>
                    </div>
      ...
      {/* ─── CONFIRM DEACTIVATE / DELETE DIALOG (STAFF) ──────────────── */}
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs animate-in fade-in duration-150">
          <div className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-[#E2E8F0] p-5 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Staff action to remove or deactivate driver record
                </p>
              </div>
            </div>
            ...
            <div className="p-3 bg-amber-50 rounded-lg border border-amber-200 text-xs text-amber-800 space-y-1">
              ...
              <p className="text-[11px] text-amber-700 leading-relaxed">
                If this driver has past deliveries or order assignments, they will be safely deactivated so historical records remain completely intact.
              </p>
            </div>
