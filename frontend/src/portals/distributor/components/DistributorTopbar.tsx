...
const TopbarContent: React.FC<DistributorTopbarProps> = ({
  ...
  return (
    <header className={`w-full bg-white border-b border-[#E2E8F0] px-3.5 sm:px-6 shrink-0 select-none ${className}`}>
      ...
      <div className="flex flex-wrap sm:flex-nowrap items-center justify-between gap-2 py-2 sm:py-2.5 min-h-[44px] sm:min-h-[56px]">
        ...
        <div className="flex items-center justify-end gap-1.5 sm:gap-2 flex-wrap sm:flex-nowrap ml-auto">
          {!hideQuickCustomer && (
            <button
              type="button"
              onClick={openQuickCustomerModal}
              className="inline-flex items-center justify-center gap-1.5 p-2 sm:px-3 sm:py-1.5 bg-[#1677C8]/10 hover:bg-[#1677C8]/20 text-[#1677C8] active:scale-95 rounded-xl text-xs font-bold transition cursor-pointer shrink-0"
              title="Quick Add Customer"
            >
              <UserPlus className="w-4 h-4 sm:w-3.5 sm:h-3.5" />
              <span className="hidden md:inline">Quick Add Customer</span>
              <span className="hidden sm:inline md:hidden">+ Customer</span>
            </button>
          )}
          {actions}
        </div>
      ...
      {secondaryRow && (
        <div className="pb-2.5 sm:pb-3 border-t border-[#F1F5F9] pt-2 sm:pt-2.5">
          {secondaryRow}
        </div>
      )}
    </header>
