    ...
    <div className={`relative inline-block ${className}`} ref={dropdownRef}>
      ...
      >
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <span className="text-[#64748B] font-medium hidden sm:inline">Period:</span>
          <span className="font-semibold text-[#16324F] truncate">{currentLabel}</span>
        </div>
        <ChevronDown className={`w-3.5 h-3.5 text-[#94A3B8] transition-transform duration-150 shrink-0 ${isOpen ? 'rotate-180' : ''}`} />
      </button>
