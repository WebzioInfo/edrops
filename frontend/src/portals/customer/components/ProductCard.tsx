...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#E2E8F0] shadow-xs flex flex-col overflow-hidden hover:shadow-md transition-all duration-300 group relative w-full h-full">
      {/* Image Area - Aspect Square on mobile (<lg), 4:3 on desktop (>=lg) for wider proportional presentation */}
      <div className="aspect-square lg:aspect-[4/3] w-full bg-[#F8FAFC] overflow-hidden flex items-center justify-center relative rounded-t-2xl">
        {imageUrl ? (
          <img
            src={imageUrl}
            alt={product.name}
        ...
        {/* Brand / Pure Badge */}
        <div className="absolute top-2 left-2 sm:top-2.5 sm:left-2.5 bg-white/95 backdrop-blur-xs text-[#1E88E5] text-[10px] sm:text-[11px] font-bold px-2 sm:px-2.5 py-0.5 rounded-full border border-sky-100 shadow-xs">
          {brandName}
        </div>
      </div>

      {/* Content Area */}
      <div className="p-2 sm:p-3 lg:p-4 flex flex-col flex-1 justify-between gap-2">
        <div>
          {/* Title - Line clamp 2 */}
          <h3
            className="text-[13px] sm:text-[15px] lg:text-[16px] font-bold text-[#0F172A] line-clamp-2 leading-snug min-h-[34px] sm:min-h-[40px]"
            title={product.name}
          >
            {product.name}
          </h3>
          ...
          <div className="mt-1 flex items-baseline gap-1">
            <span className="text-[16px] sm:text-[19px] lg:text-[22px] font-black text-[#1E88E5]">
              ₹{product.price}
            </span>
            <span className="text-[10px] sm:text-[11px] lg:text-xs font-medium text-[#64748B]">
              / unit
            </span>
          </div>
