...
export class DeliveryService {
  ...
  async submitReport(deliveryId: string, data: { deliveredQty: number; emptyCollected: number; notes?: string }) {
  ...
  async confirm(deliveryId: string, data: { deliveredQty: number; emptyCollected: number; damagedQty: number; notes?: string; staffId?: string }) {
  ...
  async findAll() {
    return this.prisma.delivery.findMany({
      include: { 
        customer: { include: { user: true, jarBalance: true, jarDeposit: true, jarOwnership: true } }, 
        address: true,
        assignment: { include: { deliveryPartner: { include: { user: true } } } },
        report: true
      },
