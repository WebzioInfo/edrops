...
export class WhatsAppProvider implements INotificationProvider {
  ...
  async send(payload: NotificationPayload): Promise<void> {
    ...
    if (!phone) {
      this.logger.debug(`No phone number provided for payload ${payload.type}. Skipping WhatsApp notification.`);
      return;
    }

    // TODO: Implement WhatsApp API Logic when infrastructure is ready
    this.logger.log(`[STUB] Sending WhatsApp Notification to phone ${phone}: ${payload.title}`);
  }
}
