...
export class WalletService {
  ...
  async addFunds(userId: string, amount: number, type: WalletTransactionType, description?: string, referenceId?: string) {
    const customer = await this.prisma.customer.findUnique({
      where: { userId }
    });
    if (!customer) throw new NotFoundException('Customer profile not found');
    ...
    return this.prisma.$transaction(async (tx) => {
      let wallet = await tx.wallet.findUnique({ where: { customerId: customer.id } });
      if (!wallet) {
  ...
  async deductFunds(userId: string, amount: number, type: WalletTransactionType, description?: string, referenceId?: string) {
    const customer = await this.prisma.customer.findUnique({
      where: { userId }
    });
    if (!customer) throw new NotFoundException('Customer profile not found');
  ...
  async ownJar(userId: string) {
    const customer = await this.prisma.customer.findUnique({
      where: { userId },
      include: { wallet: true, jarDeposit: true, jarOwnership: true, jarBalance: true }
    });
    ...
    if (!wallet) {
      wallet = await this.prisma.wallet.create({ data: { customerId: customer.id, balance: 0.00 } });
    }

    let jarDeposit = customer.jarDeposit;
    if (!jarDeposit) {
      jarDeposit = await this.prisma.jarDeposit.create({
        data: { customerId: customer.id, maxActiveJars: 0, depositPaid: 0.00, depositDue: 0.00 }
      });
    }
    ...
    const depositAmount = await this.settingsService.getSettingNumber('DEPOSIT_AMOUNT_PER_JAR', 200.0);
    ...
    if (wallet.balance < depositAmount) {
      throw new BadRequestException(`Insufficient wallet balance (₹${depositAmount} required)`);
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const newDepositDue = Math.max(0.0, jarDeposit.depositDue - depositAmount);
